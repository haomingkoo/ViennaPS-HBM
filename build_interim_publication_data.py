...
if not __debug__:
    raise RuntimeError(
        "publication validation cannot run with Python assertions disabled"
    )
...
TRAVELER_RENDER = (
    ...
    / "screening_traveler_1cf4ff64c506b271"
...
cycle_13 = next(
    row for row in source["bosch_cycle_history"]["cycles"] if row["cycle"] == 13
)
...
assert (
    layers["production_doe_authorization_scope"]
...
candidate_cases = [
    row for row in structural["cases"] if row["arm"] == "candidate_rank1_quasi_steady"
]
control_cases = [
    row
    for row in structural["cases"]
    if row["arm"] == "morphology_only_bottom_up_positive_control"
]
...
assert (
    confirmation["decision"]["classification"]
...
assert (
    boundary["reflection_arms"]["3200"]["realized_kinematic_ratio_by_tier"][
        "continuity"
    ]["stream_pass_count"]
    == 0
)
...
assert (
    boundary["reflection_arms"]["3200"]["broad_analytic_envelope"]["evaluation_status"]
...
assert all(len(digest) == 64 for digest in r1_manifest["runtime_fingerprint"].values())

full_reference_rows = [
    row for row in gate0["reviewed_cases"] if row["arm"] == "full_reference_fine"
]
assert len(full_reference_rows) == 4
full_reference_passes = [
    row for row in full_reference_rows if all(row["gates"].values())
]
full_reference_misses = [
    row for row in full_reference_rows if not all(row["gates"].values())
]
assert len(full_reference_passes) == 3
assert len(full_reference_misses) == 1
full_reference_miss = full_reference_misses[0]
depth_upper_limit = (
    gate0_manifest["target"]["etch_depth"] + gate0_manifest["target"]["depth_tolerance"]
)
    ...
    erosion_ranges.append(
        {
            ...
            "all_four_seeds_survive": arm
            in gate0["erosion_response"]["all_seed_surviving_arms"],
        }
    )
    ...
    compact_structural.append(
        {
            ...
            "initial_floor_to_lower_wall_velocity_ratio": row["initial_region_rates"][
                "floor_to_lower_wall_velocity_ratio"
            ],
            "required_floor_to_lower_wall_velocity_ratio": row[
...
data = {
    ...
    "pattern_bosch_gate0": {
        ...
        "full_vs_quarter": compact_bridge(gate0["comparisons"]["full_vs_quarter"]),
        "grid_bridge": compact_bridge(gate0["comparisons"]["grid_bridge"]),
        "mask_erosion": {
            "all_seeds_monotonic": gate0["erosion_response"]["all_seeds_monotonic"],
        ...
        "full_traveler_authorized": gate0["decision"]["full_traveler_authorized"],
        ...
        "corrective_r1": {
            ...
            "manifest_sha256": sha256(SOURCES["pattern_bosch_gate0_r1_manifest"]),
            "initial_cases": 9,
            "maximum_executed_cases": r1_manifest["mask_bracket"][
                "maximum_executed_cases"
            ],
            "screen_rays_per_point": r1_manifest["numerics"]["screen_rays_per_point"],
            "anchor_rays_per_point": r1_manifest["numerics"]["anchor_rays_per_point"],
        },
    },
    "bosch": {
    ...
    "layers": {
        ...
        "final_recipe_acceptance_authorized": layers[
        ...
        "liner_lower_wall_field_mean": fine_layers["metrics"]["liner_lower_wall_field"][
            "mean"
        ],
        "stack_min_mean": fine_layers["metrics"]["stack_min"]["mean"],
        "barrier_lower_wall_field_mean": fine_layers["metrics"][
            "barrier_lower_wall_field"
        ]["mean"],
        "seed_lower_wall_field_mean": fine_layers["metrics"]["seed_lower_wall_field"][
            "mean"
        ],
    },
    "cu_structural": {
    ...
    "cu_transport": {
        ...
        "passing_cross_tier_designs": sum(
            row["all_eight_streams_pass"] for row in transport["cross_tier_designs"]
        ),
    ...
    "cu_confirmation": {
        ...
        "class_changing_numerical_interactions": len(
            confirmation["class_changing_interactions"]
        ),
    ...
    "cu_boundary": {
        ...
        "multiresponse_paired_directions": boundary["multiresponse_paired_directions"],
        "realized_kinematic_ratio_by_tier": boundary["reflection_arms"]["3200"][
...
print(
    f"wrote {OUTPUT.name}: {data['selected_foundation_cases']} selected foundation cases"
)
