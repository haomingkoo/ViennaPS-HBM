...
def layer_thickness_metrics_full_2d(
    ...
    *,
    ...
    floor = float(tm.point_to_polyline_distances(
        [floor_point], outer_nodes, outer_lines
    )[0])
    ...
    return {
        ...
        "field_thickness_asymmetry": (
            ...
            if field_reference > 0.0 else None
        ),
...
def material_layer_metrics_full_2d(
    geometry,
    material,
    inner_nodes,
    inner_lines,
    ...
    **thickness_options,
    ...
    """Combine film geometry with material-region connectivity."""
    metrics = layer_thickness_metrics_full_2d(
        inner_nodes,
        inner_lines,
        outer_nodes,
        outer_lines,
    ...
    connectivity_field_y = float(surface_y)
    connectivity_floor_y = float(floor_y)
    connectivity = tm.material_region_connectivity_2d(
        geometry,
        material,
        floor_y=connectivity_floor_y,
        field_y=connectivity_field_y,
        via_radius=via_radius,
        grid_delta=grid_delta,
        local_minimum_thickness=metrics["minimum_local_thickness"],
        resolution_cells=resolution_cells,
    )
    return {
        **metrics,
        "connectivity_floor_anchor_y": connectivity_floor_y,
        "connectivity_field_anchor_y": connectivity_field_y,
        "material_connectivity_resolved": connectivity["resolved"],
        "material_connectivity_unresolved_reasons": connectivity[
            "unresolved_reasons"
        ],
        "material_region_connected_field_to_floor": connectivity[
            "floor_to_both_mouths_connected"
        ],
        "detached_material_fragment_count": connectivity[
            "detached_fragment_count"
        ],
        "material_connectivity": connectivity,
    }
