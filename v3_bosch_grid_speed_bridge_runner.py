"""Run the four-case Bosch grid-speed bridge."""
...
DEFAULT_MANIFEST = Path("evidence/numerical/v3_bosch_grid_speed_bridge_manifest.json")
DEFAULT_OUTPUT = Path("autoresearch-results/restart_audit/v3_bosch_grid_speed_bridge_rows.jsonl")
CAMPAIGN = "v3-bosch-grid-speed-bridge-phase-a"


def evidence_origin():
    return {
        "mode": "executed_v3_bosch_grid_speed_bridge_phase_a",
...
def expand_cases(manifest):
    ...
    source_cases = {case["case_id"]: case for case in stage2a.expand_cases(focused)}
    cases = []
    for run in manifest["design"]["runs"]:
        case = copy.deepcopy(source_cases[run["reference_case_id"]])
        if case["rng_seed"] != run["rng_seed"]:
            raise ValueError(f"{run['run_id']}: declared paired seed differs")
        case.update({
            "campaign": manifest["campaign"],
            "labels": list(manifest["labels"]),
            "recipe_id": "v3gridbridge_" + run["run_id"].replace(".", "p"),
            "design_class": "paired_grid_speed_bridge",
            "anchor_reasons": [f"paired_grid:{run['pair_id']}:{run['grid_delta']}"],
        ...
        case["numerics"]["grid_delta"] = run["grid_delta"]
        case["rng_stream"].update({
            "allocation_id": f"grid_bridge_{run['pair_id']}",
            "intentional_numerical_pair_reuse": True,
            "paired_reference_case_id": run["reference_case_id"],
        })
...
def paired_rng_errors(cases):
    errors = []
    groups = {}
    for case in cases:
        groups.setdefault(case["rng_stream"]["allocation_id"], []).append(case)
    intervals = []
    for allocation, rows in groups.items():
        if len(rows) != 2:
            errors.append(f"{allocation}: expected two numerical arms")
            continue
        if {row["numerics"]["grid_delta"] for row in rows} != {0.0025, 0.005}:
            errors.append(f"{allocation}: grid arms differ")
        seeds = {row["rng_seed"] for row in rows}
        bounds = {(row["rng_stream"]["first_process_seed"], row["rng_stream"]["last_process_seed"]) for row in rows}
        if len(seeds) != 1 or len(bounds) != 1:
            errors.append(f"{allocation}: paired seed interval differs")
        if any(row["rng_stream"].get("intentional_numerical_pair_reuse") is not True for row in rows):
            errors.append(f"{allocation}: reuse declaration is missing")
        for row in rows:
            stream = row["rng_stream"]
            for reserved in stream["reserved_prior_v3_intervals"]:
                if not (
                    ...
                    errors.append(f"{allocation} overlaps prior interval {reserved['campaign']}")
        if bounds:
            intervals.append((allocation, next(iter(bounds))))
    for index, (first_name, first) in enumerate(intervals):
        for second_name, second in intervals[index + 1:]:
            if not (first[1] < second[0] or second[1] < first[0]):
                errors.append(f"{first_name} overlaps independent pair {second_name}")
    return errors
...
def validate_manifest(manifest):
    ...
    if manifest.get("design", {}).get("analysis_plan", {}).get("accuracy_limit", "missing") is not None:
        errors.append("an accuracy limit was introduced")
    root = Path(__file__).resolve().parent
    ...
    errors.extend(paired_rng_errors(cases))
...
def main():
    ...
    if errors:
        raise ValueError("invalid grid-speed bridge: " + "; ".join(errors))
