...
def material_region_connectivity_2d(
    ...
    *,
    ...
    if local_minimum_thickness is not None:
        ...
        if not np.isfinite(local_minimum_thickness) or local_minimum_thickness < 0.0:
            raise ValueError("local_minimum_thickness must be finite and nonnegative")
    ...
    def anchor_hits(component_points) -> AnchorHits:
        ...
        near_floor = np.abs(y - floor_y) <= anchor_tolerance + comparison_tolerance
        near_field = np.abs(y - field_y) <= anchor_tolerance + comparison_tolerance
        inside_rim = np.abs(x) <= via_radius + anchor_tolerance
        return {
            "floor": bool(
                np.any(near_floor & (np.abs(x) <= floor_x_limit + comparison_tolerance))
            ),
            "left_mouth": bool(
                np.any(
                    near_field
    ...
    for component_id in negative_ids:
        component_points = points[negative & (integer_component_ids == component_id)]
    ...
    if local_minimum_thickness is not None and (
        local_minimum_thickness <= resolution_limit + comparison_tolerance
    ):
...
def line_intersections_at_x(nodes, lines, x, *, y_bounds=None, atol=1e-12):
    ...
    return line_intersections_at_y(swapped, lines, x, x_bounds=bounds, atol=atol)
...
def opening_geometry_at_y(nodes, lines, y, *, max_radius):
    ...
    xs = line_intersections_at_y(nodes, lines, y, x_bounds=(-max_radius, max_radius))
    negative = xs[xs < 0.0]
    positive = xs[xs > 0.0]
...
def pattern_metrics_2d(
    ...
    *,
    ...
    lower = opening_geometry_at_y(points, lines, lower_y, max_radius=max_radius)
    middle = opening_geometry_at_y(points, lines, middle_y, max_radius=max_radius)
    upper = opening_geometry_at_y(points, lines, upper_y, max_radius=max_radius)
    sidewall_angle = math.degrees(
        math.atan(0.5 * (upper["cd"] - lower["cd"]) / (upper_y - lower_y))
    )
    right_angle = math.degrees(
        math.atan((upper["right"] - lower["right"]) / (upper_y - lower_y))
    )
    left_angle = math.degrees(
        math.atan((lower["left"] - upper["left"]) / (upper_y - lower_y))
    )
    opening_valid = all(
...
def etch_profile_metrics_2d(
    ...
    *,
    ...
    radii = np.asarray(
        [quarter_via_radius_at_y(nodes, lines, y, max_radius=max_radius) for y in ys]
    )
    cds = 2.0 * radii
    ...
    def radius_at(fraction):
        ...
        return quarter_via_radius_at_y(nodes, lines, y, max_radius=max_radius)

    radius_top = radius_at(top_fraction)
    radius_middle = radius_at(middle_fraction)
    radius_bottom = radius_at(bottom_fraction)
    chord_slope = (radius_bottom - radius_top) / (
        (bottom_fraction - top_fraction) * depth
    )
    straight_wall = radius_top + chord_slope * (depth_positions - top_fraction * depth)
...
def measure_full_via_profile_2d(
    ...
    *,
    ...
    chord_slope = (half_widths[bottom_index] - half_widths[top_index]) / (
        (bottom_fraction - top_fraction) * depth
    )
    ...
    metrics = {
        ...
        "scallop_rms": float(np.sqrt(np.mean((half_widths - smooth_profile) ** 2))),
    ...
    *,
    ...
    exterior_index = max(
        ...
        key=lambda index: float(
            summaries[index]["bounds_max"][0] - summaries[index]["bounds_min"][0]
        ),
    )
    exterior = summaries[exterior_index]
    exterior_points = points[exterior["node_indices"], :2]
    in_via = (exterior_points[:, 0] >= x_lo - tolerance) & (
    ...
    open_void_depth = (
        ...
        if open_void and np.any(in_via)
        else 0.0
    )
    ...
    if open_void:
        ...
        open_void_area = float(np.mean(section_depths) * (x_hi - x_lo))
    ...
    for index, summary in enumerate(summaries):
        ...
        if not recognized_void:
            ...
            continue
        polygon = ordered_closed_component(points, edges, summary["node_indices"])
    ...
    return {
        ...
        "maximum_void_width": max(
            (void["width"] for void in closed_voids), default=0.0
        ),
        "maximum_void_height": max(
            (void["height"] for void in closed_voids), default=0.0
        ),
...
def cmp_profile_metrics_2d(
    *,
    ...
    seed_heights = sampled_heights(post_seed_nodes, post_seed_lines, barrier_heights)
    ...
    return {
        ...
        "invalid_reason": None
        if valid
        else ("CMP stop layer consumed" if not stop_survives else "Cu plug consumed"),
        ...
        "cu_removed_at_center": (
            ...
            if pre_cu_center_y is not None and plug_survives
            else None
        ),
