...
| What did the 25-case mask/Bosch range pilot produce? | Observed at an unqualified setting | [`build_pattern_bosch_range_pilot_review.py`](../build_pattern_bosch_range_pilot_review.py) | [`pattern_bosch_range_pilot_review.json`](../pattern_bosch_range_pilot_review.json) and the self-contained [`source bundle`](../evidence/bosch/range_pilot/source_bundle.json) | Twenty cases returned all 14 measurements. Five saved profiles could not be read by the wall-intersection measurement. Twelve controls changed together, so the pilot cannot rank factors or locate a boundary. |
| How do paired Bosch controls change the profile? | Numerically characterized | [`review_v3_bosch_cheap_interactions.py`](../review_v3_bosch_cheap_interactions.py) | 28 reviewed rows in [`evidence/numerical/v3_bosch_cheap_interactions_review.json`](../evidence/numerical/v3_bosch_cheap_interactions_review.json) and exported contours in [`bosch_tutorial_data.json`](../bosch_tutorial_data.json) | All 28 discovery cases are valid and 8 lie inside the assumed geometry bands. The single-run corners do not estimate interaction uncertainty or confirm a process window. |
| Can the tutorial replay one dry-etch run over time? | Implementation verified | [`build_bosch_trajectory_replay.py`](../build_bosch_trajectory_replay.py) | Seven replayed frames in [`bosch_trajectory_replay.json`](../bosch_trajectory_replay.json) | The final replay exactly matches the saved native checkpoint. Intermediate frames are reproducible replays, not independent native snapshots. One run does not establish a process window. |
...
| Is 250 rays accurate enough for broad screening? | Pending | [`v3_bosch_clean_ray_ladder.py`](../v3_bosch_clean_ray_ladder.py) | Earlier commissioning rows and the active clean paired ladder under `autoresearch-results/restart_audit/` | An earlier comparison changed a width measurement by 0.0178 model units, but also changed random streams and stopping. It is a warning, not a clean ray-only qualification. |
