"""Checkpointed runner for the reduced 500-ray Bosch interaction screen."""
...
DEFAULT_MANIFEST = Path(".scratch/full-traveler-autoresearch/v3_bosch_cheap_interactions_manifest.json")
DEFAULT_OUTPUT = Path("autoresearch-results/restart_audit/v3_bosch_cheap_interactions_rows.jsonl")


def evidence_origin():
    return {
        "mode": "executed_v3_bosch_cheap_interactions",
...
def completed_rows(output, cases):
    ...
    for number, line in enumerate(output.read_text().splitlines(), 1):
        ...
        if row.get("ok") is not True or row.get("evidence_origin") != evidence_origin():
            raise ValueError(f"invalid interaction row {number}")
...
def validate_manifest(manifest):
    errors = []
    if manifest.get("campaign") != "v3-bosch-cheap-interactions":
    ...
    if len(manifest.get("design", {}).get("recipes", [])) != 28:
        errors.append("interaction case count differs")
...
def main():
    ...
    if errors:
        raise ValueError("invalid cheap interaction manifest: " + "; ".join(errors))
        ...
        with futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
            ...
            for index, future in enumerate(futures.as_completed(future_to_case), 1):
                ...
                if (
                    ...
                    raise RuntimeError(f"interaction case failed: {case['case_id']}")
